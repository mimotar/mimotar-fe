...
export default function StartProjectFlow() {
  ...
  const validateStep1 = () => {
    ...
    if (!description.trim()) {
      setDescriptionError("Comprehensive Deliverables Scope is required");
      isValid = false;
    } else {
      setDescriptionError("");
    }

    // Amount validation: fine to be 0 temporarily if they specify milestones later,
    // but must be > 0 if not using milestones
    if (!hasMilestones && amount <= 0) {
      setAmountError("Total Project Value must be greater than zero");
      isValid = false;
    } else {
      setAmountError("");
    }
    ...
    if (!deadline) {
      setDeadlineError("Final Close Deadline date is required");
      isValid = false;
    } else {
      setDeadlineError("");
    }
          ...
          {hasMilestones ? (
            ...
            <div className="bg-purple-100/[0.03] border border-brand-primary/10 p-6 rounded-2xl flex flex-col gap-3 font-sans">
              ...
              <p className="text-xs text-gray-500 leading-relaxed leading-relaxed">
                You chose not to partition the payment. The total project value
                of{" "}
                <span className="font-bold text-gray-800 font-mono">
                  {formatCurrency(amount)}
                </span>{" "}
                will be locked in bulk and released entirely at once upon
                completion delivery and approval. This is super efficient for
                short-term or sprint tasks.
              </p>
            </div>
          )}
