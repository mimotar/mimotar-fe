...
import { ActiveContract, RecentActivity } from "../types/IGetDashboard";
import { formatDistanceToNow } from "date-fns";
import { QuestionIcon } from "@phosphor-icons/react";
import { formatNumberToCurrency } from "@/app/utils/formatNumberToCurrency";
import { useRouter } from "next/navigation";

interface ActiveContract_RecentLogProps {
      ...
      <div className="lg:col-span-2 space-y-4 text-left">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="text-body-sm cursor-pointer font-bold text-brand-primary hover:underline flex items-center gap-1 font-sans"
          >
            See All
        ...
        {activeContracts?.length === 0 ? (
          <div className="bg-white rounded-2xl p-12 text-center flex flex-col items-center justify-center border border-gray-50 shadow-xs animate-fade-in">
            ...
            <p className="text-body-sm text-brand-neutral mt-2 max-w-sm leading-relaxed">
              Connect with clients locally or internationally. No milestones are
              worked on without funded reserves protecting your delivery.
            </p>
            <button
              onClick={() => navigate.push("projects")}
          ...
          <div className="flex flex-col gap-3">
            {activeContracts.map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  onClick={() => navigate.push(`project-workspace/${p.id}`)}
                  ...
                  return (
                    <div className="relative">
                      ...
                      <span className="text-caption text-gray-400 font-mono block">
                        {formatDistanceToNow(new Date(recentLogsLog.time), {
                          addSuffix: true,
                        })}
                      </span>
